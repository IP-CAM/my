...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			/* add */
			if ($this->user->hasPermission('access', 'catalog/infocategory')) {		
				$catalog[] = array(
					'name'	   => $this->language->get('text_infocategory'),
					'href'     => $this->url->link('catalog/infocategory', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);					
			}
			/* end add*/
