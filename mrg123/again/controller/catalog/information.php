...
class ControllerCatalogInformation extends Controller {
	...
	protected function getForm() {
		...
		$data['entry_date_release'] = $this->language->get('entry_date_release');
		$data['entry_author'] = $this->language->get('entry_author');
		$data['entry_source'] = $this->language->get('entry_source');
		$data['entry_infocategory'] = $this->language->get('entry_infocategory');
		$data['help_infocategory'] = $this->language->get('help_infocategory');
		
		if (isset($this->request->get['information_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$information_extra = $this->model_catalog_information->getInformationExtra($this->request->get['information_id']);
		}
		
		$this->load->model('catalog/infocategory');
		if (isset($this->request->post['infocategory'])) {
			$infocategories = $this->request->post['infocategory'];
		} elseif (isset($this->request->get['information_id'])) {
			$infocategories = $this->model_catalog_information->getInfocategories($this->request->get['information_id']);
		} else {
			$infocategories = array();
		}
		$data['infocategories'] = array();
		foreach ($infocategories as $infocategory_id) {
			$infocategory = $this->model_catalog_infocategory->getInfocategory($infocategory_id);

			if ($infocategory) {
				$data['infocategories'][] = array(
					'infocategory_id' => $infocategory['infocategory_id'],
					'name'        => ($infocategory['path']) ? $infocategory['path'] . ' &gt; ' . $infocategory['name'] : $infocategory['name']
				);
			}
		}
		...
		} elseif (!empty($information_extra)) {
			$data['date_release'] = ($information_extra['date_release'] != '0000-00-00') ? $information_extra['date_release'] : '';
		} else {
		...
		if (isset($this->request->post['author'])) {
			...
			$data['author'] = $information_extra['author'];
		} else {
			$data['author'] = '';
		}
		...
		if (isset($this->request->post['source'])) {
			...
			$data['source'] = $information_extra['source'];
		} else {
			$data['source'] = '';
		}
		...
		if(isset($this->request->post['image'])){
			...
			$data['image'] = $information_extra['image'];
		} else {
			$data['image'] = '';
		}
		...
		} elseif (!empty($information_extra) && is_file(DIR_IMAGE . $information_extra['image'])){
			$data['thumb'] = $this->model_tool_image->resize($information_extra['image'],100,100);
		} else {
