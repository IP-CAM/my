                            ...
                            if (!empty($warehouses) || !empty($warehouses_products)) {
                                ...
                                if (!empty($warehouses_products)) {
                                    foreach ($warehouses_products as $wh_pr) {
                                        echo '<span class="bold text-info">' . $wh_pr->name . ': <input type="hidden" value="'.$this->sma->formatDecimal($wh_pr->quantity).'" id="vwh_qty_' . $wh_pr->id . '"><span class="padding05" id="rwh_qty_' . $wh_pr->id . '">' . $this->sma->formatQuantity($wh_pr->quantity) . '</span>' . ($wh_pr->rack ? form_input('rack_' . $wh_pr->id, $wh_pr->rack, 'class="form-control" id="rack_' . $wh_pr->id . '" placeholder="' . lang('rack') . '"'): form_input('rack_' . $wh_pr->id, $wh_pr->rack, 'class="form-control" id="rack_' . $wh_pr->id . '" placeholder="' . lang('rack') . '"')) . '</span><br>';
                                    }
                                }
