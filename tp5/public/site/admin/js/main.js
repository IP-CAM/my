$(document).ready(function(){
    ...
    $(function(){
        var pdd = new DropDown($('.wrapper-dropdown'));
	...
	if ((navigator.userAgent.indexOf('MSIE') >= 0) && (navigator.userAgent.indexOf('Opera') < 0)){
		...
		$(".wrapper-dropdown").click(function(){
			...
			$(this).find(".dropdown").fadeToggle(1000);
		})
	}
    ...
    $('#sidebar>ul>li:not(".nosubmenu")>a').click(function(){        
        var e = $(this).parent();
        if(e.hasClass('active')){
            e.removeClass('active').find('ul:first').slideUp();
...
var cookieList = function(cookieName) {
    ...
    return {
        'add': function(val, options){
            ...
            //$.cookie(cookieName, items.join(','), optionsArray);
            $.cookie(cookieName, items.join(','), optionsArray);
        },
