...
'category_id'=>$result['category_id'],
			...
			foreach ($results as $result) {
				...
				if ($this->config->get('config_tax')) {
					$tax = $this->currency->format($result['starting_from']?$result['starting_from']:((float)$result['special'] ? $result['special'] : $result['price']), $this->session->data['currency']);
				} else {
					$tax = false;
				}
...
'otp_bullets' => $otp_bullets,
	...
	$this->document->addScript('catalog/view/theme/default/script/ok_resize.js','footer');
			...
			$this->response->setOutput($this->load->view('weixin/category', $data));
		} else {
			$url = '';
