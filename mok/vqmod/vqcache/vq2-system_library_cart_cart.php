	...
	public function __construct($registry) {
		...
		if ($this->customer->getId()) {
			...
			foreach ($cart_query->rows as $cart) {
				...
				$this->add($cart['product_id'], $cart['quantity'], json_decode($cart['option']), $cart['recurring_id'], $cart['otp_id'], $cart['swap_id']);
			}
		}
	}
	...
	if ($otp_check_query->num_rows) {
		...
		foreach (json_decode($cart['option']) as $otp_option_id => $otp_option_value_id) {
			if (in_array($otp_option_id, $otp_options)) {
				$otpcount++;
				$otp_option_query = $this->db->query("SELECT o.type, od.name FROM `" . DB_PREFIX . "option` o LEFT JOIN `" . DB_PREFIX . "option_description` od ON (o.option_id = od.option_id) WHERE o.option_id = '" . $otp_option_id . "' AND od.language_id = '" . $this->config->get('config_language_id') . "'");
...
if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				foreach (json_decode($cart['option']) as $product_option_id => $value) {
					...
					if ($option_query->num_rows) {
						if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'list') {
...
$product_data[] = array(
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0, $otp_id = 0, $swap_id = 0) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "cart WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "'");

		if (!$query->row['total']) {
			$this->db->query("INSERT " . DB_PREFIX . "cart SET api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "', customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "', date_added = NOW(), `otp_id` = '" . $otp_id . "', `swap_id` = '" . $swap_id . "'");
		} else {
