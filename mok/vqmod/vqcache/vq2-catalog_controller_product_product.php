...
public function getotp() {
	...
	if ($otp) {
		$product_info = $this->model_catalog_product->getProduct($this->request->get['product_id']);
		if ($this->config->get('config_tax') && ($this->config->get('config_otp_price') && $otp['price'] > 0 || $this->config->get('config_otp_special') && $otp['special'] > 0)) { $otp['tax'] = $this->currency->format((float)($otp['special'] > 0?$otp['special']:$otp['price']), $this->session->data['currency']); }
		else { $otp['tax'] = 0; }
		if ($this->config->get('config_otp_price') && $otp['price'] > 0) {
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				$otp['price'] = $this->currency->format($this->tax->calculate($otp['price'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
			}
		}
		else {
			$otp['price'] = 0;
		...
		if ($this->config->get('config_otp_special') && $otp['special'] > 0) {
			$otp['special'] = $this->currency->format($this->tax->calculate($otp['special'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
		}
		else {
			$otp['special'] = 0;
		}
...
public function getswapimages() {
	...
	if (!empty($swap_images)) {
		$swap_popup = $this->model_tool_image->resize($swap_images[0]['image'], $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height'));
		$swap_thumb = $this->model_tool_image->resize($swap_images[0]['image'], $this->config->get($this->config->get('config_theme') . '_image_thumb_width'), $this->config->get($this->config->get('config_theme') . '_image_thumb_height'));
		...
		foreach($swap_images as $key => $swap) {
			$swap_popup = $this->model_tool_image->resize($swap['image'], $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height'));
			$swap_thumb = $this->model_tool_image->resize($swap['image'], $this->config->get($this->config->get('config_theme') . '_image_thumb_width'), $this->config->get($this->config->get('config_theme') . '_image_thumb_height'));
			$swap_additional = $this->model_tool_image->resize($swap['image'], $this->config->get($this->config->get('config_theme') . '_image_additional_width'), $this->config->get($this->config->get('config_theme') . '_image_additional_height'));
...
$data['images'][] = array(
	...
	'otp_thumb' => $this->model_tool_image->resize($product_info['image'], $this->config->get($this->config->get('config_theme') . '_image_thumb_width'), $this->config->get($this->config->get('config_theme') . '_image_thumb_height')),
...
'otp_thumb' => $this->model_tool_image->resize($result['image'], $this->config->get($this->config->get('config_theme') . '_image_thumb_width'), $this->config->get($this->config->get('config_theme') . '_image_thumb_height')),
...
if ($product_info['starting_from']) {
	$data['starting_from'] = $this->language->get('text_starting_from') . ' ' . $this->currency->format($this->tax->calculate($product_info['starting_from'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
	if ($this->config->get('config_tax')) {
		$data['tax'] = $this->currency->format($product_info['starting_from'], $this->session->data['currency']);
	} else {
		$data['tax'] = false;
	}
}
...
$data['products'][] = array(
					...
					'price'       => $price,
					'special'     => $special,
'starting_from' => ($result['starting_from']?$this->language->get('text_starting_from') . ' ' . $this->currency->format($this->tax->calculate($result['starting_from'], $result['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']):''),
'otp_bullets' => $otp_bullets,
			...
			$this->response->setOutput($this->load->view('weixin/product', $data));
		} else {
			$url = '';
