...
class ControllerPurchasePurchaseOrder extends Controller{
	...
	public function insert_purchase_order()
	{
		...
		if((in_array("--products--",$data['products'])) || (in_array("--Product Options--",$data['options'])) || (in_array("--Option Values--",$data['option_values'])) || (count($data['quantity']) != count(array_filter($data['quantity']))) || (count($data['options']) != count(array_filter($data['options']))) || (count($data['products']) != count(array_filter($data['products']))) || (count($data['option_values']) != count(array_filter($data['option_values']))))
		{
			$data['form_bit'] = 0;
		...
		else
		{
			...
			if(isset($this->request->post['mail_bit']))
			{
				...
				$mail->Port       = 587;                  
				$mail->Username   = "gresystechnologies@gmail.com";  
				$mail->Password   = "Gresys@Gresys";            
