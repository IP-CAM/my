...
class ModelPurchasePurchaseOrder extends Model {
	public function insert_purchase_order($data = array()){
		...
		{
			$this->db->query("INSERT INTO oc_po_product (product_id,name,quantity,order_id)	VALUES(".$data['products'][$i][0].",'".$data['products'][$i][1] . "'," . $data['quantity'][$i].",".$order_id.")");
		...
		if($data['supplier_id'] != "--Supplier--")
		{
			for($i = 0; $i<count($data['products']); $i++)
			{
				$query = $this->db->query("INSERT INTO oc_po_receive_details (quantity,product_id,supplier_id,order_id) VALUES(".$data['quantity'][$i].",".$product_ids[$i].",".$data['supplier_id'].",".$order_id.")");
			}
		}
		else{
			for($i = 0; $i<count($data['products']); $i++)
			{
				$query = $this->db->query("INSERT INTO oc_po_receive_details (product_id,supplier_id,order_id) VALUES(".$product_ids[$i].",-1,".$order_id.")");
			}
		}
		...
		else
		{
			...
			for($j = 0; $j<count($received_order_info['received_product_ids']); $j++)
			{
				for($k = 0; $k<$inner_loop_limit; $k++)
				{
					...
					{
						$this->db->query("INSERT INTO oc_po_receive_details (quantity,price,product_id,supplier_id,order_id) VALUES(".$received_order_info['received_quantities'][$k].",".$received_order_info['prices'][$k].",".$received_order_info['received_product_ids'][$j].",".$received_order_info['suppliers_ids'][$k].",".$order_id.")");
