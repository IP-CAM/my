...
class ControllerBlogBlog extends Controller {
	...
	public function index() {
		...
		if ($blog_info) {
			...
			$this->document->addStyle('catalog/view/theme/default/css/vedio_play.css');
			...
			$this->document->addScript('catalog/view/theme/default/script/ok_article.js','footer');
			...
			$data['write_comment'] = $this->url->link('blog/blog/write', 'blog_id=' . $blog_id , true);
			// Video
			$video = $this->model_blog_blog->getVideoByBlogId($blog_id);
			if(isset($video['video_path']) && $video['video_path']){
				$data['video_path'] = $video['video_path'];
			}else{
				$data['video_path'] = '';
			}
			$data['text_no_support'] = $this->language->get('text_no_support');
            ...
            if ($this->customer->isLogged()) {
                $data['is_collect'] = $this->model_account_collect_article->is_collect($this->request->get['blog_id']);
            }else{
                $data['is_collect'] = 2;
            }
	...
	public function comment() {
		...
		foreach ($results as $result) {
            ...
            if(isset($customer_info['custom_field'])){
                $customer_field_info = json_decode($customer_info['custom_field'],true);
            }

            if (isset($customer_field_info[8]) && $customer_field_info[8]){

                $image = $this->model_tool_image->resize($customer_field_info[8], $this->config->get($this->config->get('config_theme') . '_image_related_width'), $this->config->get($this->config->get('config_theme') . '_image_related_height'));
	...
	public function write() {
        ...
        if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('blog/blog/write', 'blog_id='.$this->request->get['blog_id'], true);
        ...
        if(isset($this->request->get['blog_id'])){
            ...
            $is_exist = $this->model_blog_blog->blogIsExist($this->request->get['blog_id']);
            if(!$is_exist){
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			*/
                ...
                $this->request->post['name']=$customer_field_info[9];

                $this->model_blog_comment->addComment($this->request->get['blog_id'], $this->request->post);

                $this->response->redirect($this->url->link('blog/blog', 'blog_id='.$this->request->get['blog_id'] , true));
		}

		$data['action'] = $this->url->link('blog/blog/write', 'blog_id='.$this->request->get['blog_id'] , true);

        $this->document->addStyle('catalog/view/theme/default/css/evaluate.css');

        $this->document->addScript('catalog/view/theme/default/script/zepto.min.js','footer');
        $this->document->addScript('catalog/view/theme/default/script/ok_blog_comment.js','footer');
        ...
        $this->response->setOutput($this->load->view('weixin/blog_comment', $data));
	}
