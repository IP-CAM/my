...
<% _.each(model.config.fields,  function(f){ %>
	<% if(model[model.config.id][f.id] !== undefined || f.type == "heading" || f.type == "label"){ %>
		...
		<div class="weui-cell<%= display ? '' : ' hidden' %><%= f.class ? ' '+f.class : '' %><%= require ? ' required' : '' %>" id="<%= model.config.id %>_<%= f.id %>_input" data-sort="<%= f.sort_order %>">
			<div class="weui-cell__hd"><%= htmlDecode(f.title) %></div>
            <div class="weui-cell__bd">
                <input class="ok_input" type="<%= f.type %>" name="<%= model.config.id %>.<%= f.id %>" id="<%= model.config.id %>_<%= f.id %>" value="<%= model[model.config.id][f.id] %>" <% if(Number(config.design.placeholder)) {  %>placeholder="<%= require ? '*' : '' %> <%= htmlDecode(f.title).replace(':', '') %>"<% } %> <%= setValidateRules(f.error) %> />
            </div>
		</div>
