				...
				<?php if($feild['key']=='firstname'){ ?>
					...
					<div class="form-group <?php echo $class1; ?> <?php echo ($feild['required'] ? 'required' : ''); ?>">
						<label class="control-label" for="input-shipping-firstname"><?php echo $feild['label']; ?></label>
							<input type="text" name="delivery_details[firstname]" value="" placeholder="<?php echo $feild['placeholder']; ?>" id="input-shipping-firstname" class="formcontrol" />
					</div>
				<?php } ?>
				 ...
				 <?php if($feild['key']=='lastname'){ ?>
					...
					<div class="form-group <?php echo $class1; ?> <?php echo ($feild['required'] ? 'required' : ''); ?>">
						...
						<input type="text" name="delivery_details[lastname]" value="" placeholder="<?php echo $feild['placeholder']; ?>" id="input-shipping-lastname" class="formcontrol" />
					</div>
					<?php } ?>
...
$('#shipping-existing select[name=\'delivery_details[address_id]\']').on('change', function(){
	...
	$.ajax({
		...
		dataType: 'html',
		beforeSend: function(){
			$('#onepagecheckout .ext-delivery-method').html('<div class="loader delivery-method-loader text-center"><i class="fa fa-spinner fa-pulse fa-5x fa-fw"></i><span class="sr-only">Loading...</span></div>');
			$('#onepagecheckout #button-loggedorder').attr('disabled', true);
		},
		complete: function(){
			$('#onepagecheckout #button-loggedorder').attr('disabled', false);
		},
