...
public function array_merge_r_d() {
	...
	*/
	...
	public function getConfigSetting($id, $config_key, $store_id, $config_file = false, $customer_group_id = 0){
		...
		$result['step']['confirm']['fields']['agree']['error'][0]['information_id'] = $this->config->get('config_checkout_id');				/* add customer_group_id in getCustomFieldsConfigData */		if($this->customer->isLogged()){			$customer_group = $this->customer->getGroupId();		}else{			$customer_group = 0;		}		/* end add */		
		$result['step']['payment_address']['fields'] = $result['step']['payment_address']['fields'] + $this->model_d_quickcheckout_custom_field->getCustomFieldsConfigData('account',$customer_group);
		$result['step']['payment_address']['fields'] = $result['step']['payment_address']['fields'] + $this->model_d_quickcheckout_custom_field->getCustomFieldsConfigData('address',$customer_group);
		$result['step']['shipping_address']['fields'] = $result['step']['shipping_address']['fields'] + $this->model_d_quickcheckout_custom_field->getCustomFieldsConfigData('address',$customer_group);
