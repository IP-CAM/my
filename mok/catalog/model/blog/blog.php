...
class ModelBlogBlog extends Model {
	...
	public function getVideoByBlogId($blog_id){
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "blog_ext` WHERE blog_id = '" . (int)$blog_id . "'");
		return $query->row;
	}

    public function getBlogsByManufacturer($manufacturer_id) {

        $query = $this->db->query("SELECT DISTINCT b.image , bd.blog_id , bd.title  FROM " . DB_PREFIX . "blog b LEFT JOIN ".DB_PREFIX."blog_description bd ON b.blog_id = bd.blog_id WHERE b.manufacturer_id = ".(int)$manufacturer_id."");
    ...
    public function blogIsExist($blog_id) {

        $query = $this->db->query("SELECT blog_id  FROM " . DB_PREFIX . "blog WHERE blog_id = ".(int)$blog_id."");
