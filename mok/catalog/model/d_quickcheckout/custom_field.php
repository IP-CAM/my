...
class ModelDQuickcheckoutCustomField extends Model {
	...
	public function getCustomFieldsConfigData($location, $customer_group = 0){
		...
		foreach($results as $key => $custom_field){
			...
			*/
			$custom_fields['custom_field.'.$custom_field['custom_field_id']] = array(
				'id' => 'custom_field.'.$custom_field['custom_field_id'],
			...
			if(!empty($custom_field['custom_field_value'])){
				...
				foreach($custom_field['custom_field_value'] as $option){
					...
					*/
					$custom_fields['custom_field.'.$custom_field['custom_field_id']]['options'][] = array(
						'name' =>  $option['name'],
	...
	public function getCustomFieldsSessionData($step, $location, $customer_group = 0){
		...
		foreach($results as $key => $custom_field){
			if($location == 'address'){
				...
				*/
				$custom_fields['custom_field.'.$custom_field['custom_field_id']] = (isset($this->session->data[$step]['custom_field'][$custom_field['custom_field_id']])) ? $this->session->data[$step]['custom_field'][$custom_field['custom_field_id']] : $custom_field['value'];
			}
		}
	...
	public function setCustomFieldsDefaultSessionData($location, $customer_group = 0){
		...
		foreach($results as $key => $custom_field){
			...
			$custom_fields[$custom_field['custom_field_id']] = (isset($custom_field['value'])) ? $custom_field['value']:'';
		}
