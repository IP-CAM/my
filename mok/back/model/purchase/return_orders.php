...
class ModelPurchaseReturnOrders extends Model {
	...
	public function save_return_order($data)
	{
		...
		$query2 = $this->db->query("INSERT INTO oc_po_return (order_id,product_id,supplier_id,return_quantity,reason,return_date,user_id) VALUES(".$data['order_id'].",".$data['product_id'].",".$data['supplier'].",".$data['return_quantity'].",'". $data['reason'] ."','".date('Y-m-d')."',".$this->session->data['user_id'].")");
