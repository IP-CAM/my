...
class ControllerAccountRegister extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$activity_data = array(
				...
				'name'        => $this->customer->getId().'_'.$this->request->post['email']
			);
	...
	public function validate() {		
        ...
        $len = strlen($this->request->post['lastname']);
        if($len > 15 || $len < 3 || preg_match("/\s+|^c:\\con\\con|[%,\*\"\s\<\>\&]|$guestexp/is", $this->request->post['lastname'])) {
		...
		if ($this->model_account_customer->getTotalCustomersByLastname($this->request->post['lastname'])) {
			$this->error['lastname'] = $this->language->get('error_exists_user');
		}
