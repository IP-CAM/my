...
class ControllerPaymentAlipayGuarantee extends Controller {
	...
	public function form() {
        ...
        $receive_name = $order_info['firstname'];

        $receive_address = $order_info['shipping_country'].$order_info['shipping_zone'].$order_info['shipping_city'].$order_info['shipping_address_1'] .$order_info['shipping_company'];
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/alipay_guarantee.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/alipay_guarantee.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/payment/alipay_guarantee.tpl', $data));
		}
		
	}

	public function redirect() {
		return $this->url->link('payment/alipay_guarantee/form');
	}
