	...
	public function add() {
		...
		if ($product_info) {
			...
			if (!$json) {
				...
				$json['total_mobile'] = sprintf($this->language->get('text_items_mobile'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0) + $this->gift->countProducts());

				$json['total'] = sprintf($this->language->get('text_items'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0) + $this->gift->countProducts(), $this->currency->format($total));
				...
				if ($product_info['image']) {
					...
					$image = $this->model_tool_image->resize('no-img.jpg', 80, 80);;
				}
				
				$json['product_image'] = $image;
