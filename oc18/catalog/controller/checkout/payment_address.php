...
class ControllerCheckoutPaymentAddress extends Controller {
	public function index() {
		...
		$data['ship_zone'] = $this->language->get('ship_zone');
		$data['ship_address'] = $this->language->get('ship_address');
		$data['ship_info'] = $this->language->get('ship_info');
		
		$data['store_name'] = $this->language->get('store_name');
		$data['store_address'] = $this->language->get('store_address');
		$data['store_telphone'] = $this->language->get('store_telphone');
		$data['store_work'] = $this->language->get('store_work');
		...
		if (!empty($this->session->data['shipping_method'])) {
			...
			if (!empty($shipping[1]) && isset($this->session->data['shipping_methods'][$shipping[0]]['quote'][$shipping[1]])) {
				$code = $shipping[1];
			} else {
				$code = '';
			}
			
			$stores = $this->config->get($code . '_store')?$this->config->get($code . '_store'):array();
			$customs = $this->config->get($code . '_custom');
		...
		$data['postcode_required'] = $country['postcode_required'];
			...
			if (isset($this->request->post['payment_address']) && $this->request->post['payment_address'] == 'existing') {
				...
				if (!$json) {
					...
					$activity_data = array(
						...
						'name'        => $this->customer->getId() . '_' . $this->customer->getEmail()
					);
