	...
	public function index() {
		...
		if (isset($this->session->data['payment_address'])) {
			...
			if (isset($this->session->data['shipping_method']['code'])) {
				...
				$payments = $this->config->get($code[1].'_payment');
			...
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status') && in_array($result['code'], $payments)) {
					...
					if ($method) {
						...
						$method['description'] = isset($description[$this->config->get('config_language_id')])?$description[$this->config->get('config_language_id')]:'';
			...
			if (!isset($this->session->data['payment_method']) || !in_array($this->session->data['payment_method']['code'], $codes)) {
				$this->session->data['payment_method'] = reset($method_data);
			}
		}
		...
		if ($data['code']) {
			$data['payment_info'] = nl2br($this->config->get($data['code'].'_bank' . $this->config->get('config_language_id')));
		} else {
			$data['payment_info'] = '';
		}
