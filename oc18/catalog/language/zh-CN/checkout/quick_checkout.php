<?php
// Heading
$_['heading_title']                  = '快速结账';

// Text
$_['text_cart']                      = '购物车';
$_['text_checkout_option']           = '第 1 步 结账选项';
$_['text_checkout_account']          = '收货地址';
$_['text_checkout_payment_address']  = '配送地址';
$_['text_checkout_shipping_address'] = '第 4 步 货运地址';
$_['text_checkout_shipping_method']  = '配送方式';
$_['text_checkout_payment_method']   = '支付方式';
$_['text_checkout_confirm']          = '确认订单';
$_['text_modify']                    = '更新 &raquo;';
$_['text_new_customer']              = '新客户';
$_['text_returning_customer']        = '注册用户';
$_['text_checkout']                  = '结账选项';
$_['text_i_am_returning_customer']   = '我已注册账户';
$_['text_checkout_text']             = '无需注册会员，直接下订单快速结账购买商品。';
$_['text_login']                     = '登入账户';
$_['text_login_text']                = '如果您已经是本站会员，请按此直接登录。';
$_['text_register']                  = '注册账户';
$_['text_register_text']             = '注册一个账户以便您更快捷地购物， 查看订单状态， 查看订购记录， 和更多的管理项目。 您将能赚取购物积分丶购物积分可以在购买时使用换现金。<br />快速注册：只需5秒';
$_['text_guest']                     = '直接结账';
$_['text_register_account']          = '注册一个账户以便您更快捷地购物， 查看订单状态， 查看采购的历史记录， 和更多的管理项目。';
$_['text_forgotten']                 = '忘了密码';
$_['text_items']                     = '%s 件商品 - %s';
$_['text_your_details']              = '我的个人信息';
$_['text_your_address']              = '我的地址';
$_['text_your_password']             = '我的密码';
$_['text_agree']                     = '我已经阅读并同意 <a class="colorbox" href="%s" alt="%s"><b>%s</b></a> 条款';
$_['text_address_new']               = '添加一个新地址';
$_['text_address_existing']          = '使用现存地址';
$_['text_shipping_method']           = '请选择一个货运方式。';
$_['text_shipping_zone']             = '配送区域';
$_['text_payment_method']            = '请选择一个支付方式。';
$_['text_comments']                  = '添加订单备注';
$_['text_recurring']                 = '分期项目';
$_['text_payment_recurring']           = '分期付款';
$_['text_trial_description']         = '%s 每次 %d %s(s) 为 %d 付款(s) 然後';
$_['text_payment_description']       = '%s 每次 %d %s(s) 为 %d 付款(s)';
$_['text_payment_cancel']                     = '%s 每 %d %s(s) 直到取消';
$_['text_day']                       = '天';
$_['text_week']                      = '周';
$_['text_semi_month']                = '半月';
$_['text_month']                     = '月';
$_['text_year']                      = '年';

// Column
$_['column_name']                    = '商品名称';
$_['column_model']                   = '型号';
$_['column_quantity']                = '数量';
$_['column_price']                   = '价格';
$_['column_total']                   = '合计';

// Entry
$_['entry_email_address']            = '邮件地址';
$_['entry_email']                    = 'E-Mail';
$_['entry_password']                 = '密码';
$_['entry_confirm']                  = '确认密码';
$_['entry_firstname']                = '您的姓名';
$_['entry_lastname']                 = '您的姓氏';
$_['entry_telephone']                = '电话';
$_['entry_fax']                      = '传真';
$_['entry_address']                  = '选择地址';
$_['entry_company']                  = '&nbsp;&nbsp;&nbsp;公司名称';
$_['entry_customer_group']           = '公司类型';
$_['entry_address_1']                = '地址';
$_['entry_address_2']                = '&nbsp;&nbsp;&nbsp;地址 2';
$_['entry_postcode']                 = '邮编';
$_['entry_city']                     = '城市';
$_['entry_country']                  = '国家';
$_['entry_zone']                     = '地区 / 省份';
$_['entry_newsletter']               = '我希望订阅 %s 咨询。';
$_['entry_shipping'] 	             = '我的货运地址和运单地址相同。';
$_['entry_voucher']                  = '使用礼品券代码';
$_['entry_coupon']                   = '使用优惠券代码';
$_['entry_point']                    = '使用积分减去金额';

// Help
$_['help_point']                     = '可用%s点';

// Ship
$_['ship_zone'] 	                 = '取货点区域';
$_['ship_address'] 	                 = '取货点地址';
$_['ship_info'] 	                 = '营业时间：';
$_['custom_info'] 	                 = '交货时间：';

// Store
$_['store_name'] 	                 = '门店';
$_['store_address'] 	             = '门店地址';
$_['store_telphone'] 	             = '联系电话';
$_['store_work'] 	                 = '营业时间';

// Error
$_['error_warning']                  = '在结算时出现了问题！ 如果此问题继续存在请使用其它支付方式或<a href="%s">联系我们</a>。';
$_['error_login']                    = '警告：邮件地址或是密码不符。';
$_['error_attempts']                 = '警告: 您的账户已经超过最大尝试登录次数，请在一小时後再试。';
$_['error_approved']                 = '警告：您的帐户需要审核，才可以登录。';
$_['error_exists']                   = '警告：该邮箱地址已被注册丶请换一个电子邮箱！';
$_['error_firstname']                = '姓名必须在 1 至 32 字符之间！';
$_['error_lastname']                 = '姓氏必须在 1 至 32 字符之间！';
$_['error_email']                    = '无效的邮箱地址！';
$_['error_telephone']                = '电话号码必须为 3 至 32 字节！';
$_['error_password']                 = '密码必须在 3 至 20 字符之间！';
$_['error_confirm']                  = '确认密码与输入密码不一致！';
$_['error_address_1']                = '您还没有输入配送地址！';
$_['error_city']                     = '城市名必须在 3 至 128 字节之间！';
$_['error_postcode']                 = '邮编必须在 2 至 10 字符之间！';
$_['error_country']                  = '请选择一个国家！';
$_['error_zone']                     = '请选择地区/省份！';
$_['error_agree']                    = '警告：您未同意条款 %s！';
$_['error_address']                  = '警告：请选择一个地址！';
$_['error_shipping']                 = '警告：请选择一个货运方式！';
$_['error_no_shipping']              = '无可用货运方式。 请<a href="%s">联系我们</a> 以获取帮助！';
$_['error_payment']                  = '警告：请选择一个支付方式！';
$_['error_no_payment']               = '警告：无可用支付方式。 请<a href="%s">联系我们</a> 以获取帮助！';
$_['error_custom_field']             = '%s 必填！';
$_['error_shipping_zone']            = '请选择一个配送区域！';