...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', firstname = '" . $this->db->escape($data['lastname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', newsletter = '1', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '1', approved = '" . (int)!$customer_group_info['approval'] . "', date_added = NOW()");
		...
		$this->model_sma_sma->toSmaCustomer($data, 'customer_add');
	...
	public function editCustomer($data) {
		...
		$this->load->model('sma/sma');
		$data['customer_id'] = $customer_id;
		$this->model_sma_sma->toSmaCustomer($data, 'customer_edit');
	}
	...
	public function getCustomerByEmail($email) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer
									WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) . "'
										OR LOWER(lastname) = '" . $this->db->escape(utf8_strtolower($email)) . "'");

		return $query->row;
	}

	public function getCustomerByLastname($lastname) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE LOWER(lastname) = '" . $this->db->escape(utf8_strtolower($lastname)) . "'");
	...
	public function getTotalCustomersByLastname($lastname) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE LOWER(lastname) = '" . $this->db->escape(utf8_strtolower($lastname)) . "'");
