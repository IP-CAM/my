...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($this->request->cookie['geoAd'])) {
			$geo_ad = $this->request->cookie['geoAd'];
			
			$geo_ad_id = $geo_ad['geo_ad_id'];
			
			if (!empty($geo_ad['url'])) {
				$geo_ad = $geo_ad['name'] . ' > ' . $geo_ad['title'] . ' > ' . $geo_ad['url'];
			} else {
				$geo_ad = $geo_ad['name'] . ' > ' . $geo_ad['title'] . ' > ' . 'other';
			}
		} else {
			$geo_ad_id = '';
			$geo_ad = '';
		...
		if (isset($this->request->cookie['marketLink'])) {	
			$link = $this->request->cookie['marketLink'];
			$link_id = $link['link_id'];
			
			if (!empty($link['url'])) {
				$link_name = $link['name'] . ' > ' . $link['url'];
			} else {
				$link_name = $link['name'] . ' > ' . 'other';
			}
		} else {
			$link_id = '';
			$link_name = '';
		...
		if (isset($data['totals'])) {
			...
			foreach ($data['totals'] as $total) {
				...
				if (method_exists($this->{'model_total_' . $total['code']}, 'confirm')) {
					$this->{'model_total_' . $total['code']}->confirm($order_info, $total);
				}
			}
		}
		...
		$this->model_sma_sma->toSmaOrder($data, 'order_add');
	...
	public function editOrder($order_id, $data) {
		...
		$this->load->model('sma/sma');
		$data['order_id'] = $order_id;
		$this->model_sma_sma->toSmaOrder($data, 'order_edit');
	}
	...
	public function deleteOrder($order_id) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_action` WHERE order_id = '" . (int)$order_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_product` WHERE order_id = '" . (int)$order_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_option` WHERE order_id = '" . (int)$order_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_voucher` WHERE order_id = '" . (int)$order_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_total` WHERE order_id = '" . (int)$order_id . "'");
