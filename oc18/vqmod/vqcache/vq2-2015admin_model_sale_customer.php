...
class ModelSaleCustomer extends Model {
	public function addCustomer($data) {
		...
		if (isset($data['address'])) {
			foreach ($data['address'] as $address) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id . "', firstname = '" . $this->db->escape($address['firstname']) . "', lastname = '" . $this->db->escape($address['lastname']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "', address_2 = '" . $this->db->escape($address['address_2']) . "', city = '" . $this->db->escape($address['city']) . "', postcode = '" . $this->db->escape($address['postcode']) . "', country_id = '" . (int)$address['country_id'] . "', zone_id = '" . (int)$address['zone_id'] . "', custom_field = '" . $this->db->escape(isset($address['custom_field']) ? serialize($address['custom_field']) : '') . "'");
	...
	public function deleteCustomer($customer_id) {
		...
		$this->model_sma_sma->toSmaCustomer($data, 'customer_delete');
...
if (!empty($data['filter_telephone'])) {
	...
	public function addReward($customer_id, $description = '', $points = '', $order_id = 0) {
		...
		if ($customer_info) {
			...
			// sma
			$this->load->model('sma/sma');
			$this->model_sma_sma->toSmaCustomerPoints($customer_id);
	...
	public function deleteReward($order_id) {
		// sma
		$this->load->model('sma/sma');
		$this->model_sma_sma->deltePointsByOrderId($order_id);
