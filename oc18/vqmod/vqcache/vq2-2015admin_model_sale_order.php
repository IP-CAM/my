...
class ModelSaleOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'comment_hide'            => $order_query->row['comment_hide'],
				...
				'os'                      => $order_query->row['os'],
				'geo_ad'                  => $order_query->row['geo_ad'],
				'geo_ad_id'               => $order_query->row['geo_ad_id'],
				'link'                    => $order_query->row['link'],
				'link_id'                 => $order_query->row['link_id'],
				'date_added'              => $order_query->row['date_added'],
				'date_modified'           => $order_query->row['date_modified'],
				
				'action_viewed'           => $order_query->row['action_viewed'],
				'action_invoice'          => $order_query->row['action_invoice'],
				'action_express'          => $order_query->row['action_express'],
				'action_shipping'         => $order_query->row['action_shipping'],
				'auto_express'            => $order_query->row['auto_express'],
				'auto_shipping'           => $order_query->row['auto_shipping']
			);
		} else {
			return;
		}
	...
	public function getOrders($data = array()) {
		...
		if (!empty($data['filter_order_id'])) {			
			...
			$filter_order_ids = explode(',', $data['filter_order_id']);

			foreach ($filter_order_ids as $filter_order_id) {
				$implode[] = "o.order_id LIKE '%" . (int)$filter_order_id . "%'";
			}
	...
	public function getOrderProducts($order_id) {
			$query = $this->db->query("SELECT *, (SELECT image FROM " . DB_PREFIX . "product p WHERE p.product_id = op.product_id) AS image FROM " . DB_PREFIX . "order_product op WHERE op.order_id = '" . (int)$order_id . "'");
	...
	public function delOrderHistory($order_history_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_history` WHERE order_id = (SELECT `order_id` FROM `" . DB_PREFIX . "order_history` WHERE `order_history_id` = '".(int)$order_history_id."' LIMIT 1) ORDER BY date_added DESC LIMIT 2");
		
		$history = $query->rows[1];
		
		if ($history) {
			$this->db->query("DELETE FROM `" . DB_PREFIX . "order_history` WHERE order_history_id = '" . (int)$order_history_id . "'");
		
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int)$history['order_status_id'] . "', date_modified = NOW() WHERE order_id = '" . (int)$history['order_id'] . "'");
		}
	}
	...
	public function getTotalOrdersByTelephone($telephone, $type = 'total') {
		$status = array();
	...
	public function updateOrderAction($order_id, $type, $status) {
		$filter = array(
			'viewed'       => 'action_viewed',
			'invoice'      => 'action_invoice',
			'express'      => 'action_express',
			'shipping'     => 'action_shipping',
			'autoexpress'  => 'auto_express',
			'autoshipping' => 'auto_shipping'
		);
		...
		foreach ($filter as $key => $value) {
			if ($key == $type) {
				$set = '`'.$this->db->escape($value).'` = '.(int)$status;
				break;
			}
		}
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "order_action` SET status = '".(int)$status."', order_id = '" . (int)$order_id . "', type = '".$this->db->escape($type)."', date_added = NOW()");
	}
}