...
class Currency {
	...
	public function __construct($registry) {
		...
		if (!isset($this->session->data['ip_code'])) {
			$this->session->data['ip_code'] = $this->getInfo();
		}

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "currency");
		
		$currencies = $query->rows;
		...
		foreach ($currencies as $result) {
			$codes = explode(',', $result['country']);
			
			if ($result['status'] && in_array($this->session->data['ip_code'], $codes)) {
