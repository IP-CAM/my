...
class ControllerModuleOauth extends Controller {
	...
	public function index() {   
		...
		$data['entry_c_email'] = $this->language->get('entry_c_email');
		$data['entry_c_name'] = $this->language->get('entry_c_name');
		$data['entry_c_nickname'] = $this->language->get('entry_c_nickname');
		$data['entry_c_openid'] = $this->language->get('entry_c_openid');
		$data['entry_c_date'] = $this->language->get('entry_c_date');
		...
		$data['error_jump'] = $this->language->get('error_jump');
		...
		$data['delete'] = html_entity_decode($this->url->link('module/oauth/delete', 'token=' . $this->session->data['token'], 'SSL'), ENT_QUOTES, 'UTF-8');
		$data['action'] = $this->url->link('module/oauth', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['oauth'])) {
			$data['oauth'] = $this->request->post['oauth'];
		} elseif ($this->config->get('oauth')) { 
			$data['oauth'] = $this->config->get('oauth');
		} else {
			$data['oauth'] = '';
		}
		...
		$oauths = $this->model_module_oauth->getOauths($filter);
		
		$data['oauth_list'] = array();
		...
		$totalOauths = $this->model_module_oauth->getTotalOauths($filter);
		...
		$this->response->setOutput($this->load->view('module/oauth.tpl', $data));
	}
