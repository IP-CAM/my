...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		if (!empty($this->request->get['category_id']) && isset($_COOKIE['category_name'])) {
			$data['category_name'] = $_COOKIE['category_name'];
		} else {
			$data['category_name'] = '';
		}
	...
	protected function getForm() {
		...
		$data['entry_image_http'] = $this->language->get('entry_image_http');
		...
		if (isset($this->request->post['image_http'])) {
			$data['image_http'] = $this->request->post['image_http'];
		} elseif (!empty($product_info)) {
			$data['image_http'] = $product_info['image_http'];
		} else {
			$data['image_http'] = 1;
		}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model'])) {
			...
			foreach ($results as $result) {
				...
				if (is_file(DIR_IMAGE . $result['image'])) {
					$image = $this->model_tool_image->resize($result['image'], $size, $size);
				} else {
				...
				$json[] = array(
					...
					'option'     => $option_data,
					'price'      => $result['price'],
					'product_type'=> isset($result['warehouse_type'])?$result['warehouse_type']:''
				);
			}
		}
