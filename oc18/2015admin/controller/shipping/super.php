...
class ControllerShippingSuper extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$supers = $this->model_setting_setting->getSetting('super');
			$supers['super_status'] = $this->request->post['super_status'];
			$supers['super_sort_order'] = (int)$this->request->post['super_sort_order'];
			
			$this->model_setting_setting->editSetting('super', $supers);
	...
	public function form() {
		...
		if (isset($this->request->post['super'.$shipping_id.'_store'])) {
			$super_stores = $this->request->post['super'.$shipping_id.'_store'];
		} elseif ($this->config->get('super'.$shipping_id.'_store') && is_array($this->config->get('super'.$shipping_id.'_store'))) {
			$super_stores = $this->config->get('super'.$shipping_id.'_store');
		} else {
			$super_stores = array();
		}
		
		foreach ($super_stores as $k => $store) {
			$sort_order = array();
			
			foreach ($store['store'] as $key => $value) {
				$sort_order[$key] = (int)(!empty($value['sort_order'])?$value['sort_order']:'0');
				
				$store['store'][$key]['sort_order'] = (int)(!empty($value['sort_order'])?$value['sort_order']:'0');
			}
			
			array_multisort($sort_order, SORT_ASC, $store['store']);
			
			$super_stores[$k]['store'] = $store['store'];
		}
		
		$data['super'.$shipping_id.'_store'] = $super_stores;
	...
	public function save() {
		if (isset($this->request->post['super_id'])) {
			$id = $this->request->post['super_id'];
			
			unset($this->request->post['super_id']);
		} else {
			$id = '';
		}
		...
		if ($id) {
			...
			if (isset($supers['super']) && is_array($supers['super'])) {				
				$key = array_search($id, $supers['super']);
			} else {
				$key = false;
			}
