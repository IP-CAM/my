...
class ControllerLocalisationLocation extends Controller {
	...
	protected function getForm() {
		...
		$data['entry_googlemaps'] = $this->language->get('entry_googlemaps');
		...
		if (isset($this->error['googlemaps'])) {
			$data['error_googlemaps'] = $this->error['googlemaps'];
		} else {
			$data['error_googlemaps'] = '';
		}
		...
		} elseif (!empty($location_info) && !empty($location_info['lat'])) {
			$data['lat'] = $location_info['lat'];
		} else {
			$data['lat'] = '22.325623731150902';
		}

		if (isset($this->request->post['lng'])) {
			$data['lng'] = $this->request->post['lng'];
		} elseif (!empty($location_info) && !empty($location_info['lng'])) {
			$data['lng'] = $location_info['lng'];
		} else {
			$data['lng'] = '114.17781829833984';
		}
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['lat']) < 1) || (utf8_strlen($this->request->post['lng']) < 1)) {
			$this->error['googlemaps'] = $this->language->get('error_googlemaps');
		}
