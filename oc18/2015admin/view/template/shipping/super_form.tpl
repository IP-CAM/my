			  ...
			  <div class="table-responsive">
				<table id="store<?php echo $shipping_id; ?>-address<?php echo $store_row; ?>" class="table table-striped table-bordered table-hover">
				  ...
				  <tbody>
					...
					<tr>
					  <td class="text-left"><input type="text" name="super<?php echo $shipping_id; ?>_store[<?php echo $store_row; ?>][store][<?php echo $s_row; ?>][sort_order]" value="<?php echo $s['sort_order']; ?>" placeholder="<?php echo $entry_sort_order; ?>" class="form-control" /></td>
					  <td class="text-left">
					  <?php foreach ($languages as $language) { ?>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
       <button type="button" data-toggle="tooltip" class="btn btn-primary" data-original-title="保存配送"
	   onclick="saveShipping('<?php echo $shipping_id; ?>');">
	 	  <i class="fa fa-save"></i> 保存配送
	   </button>
		<input type="hidden" name="super_id" value="<?php echo $shipping_id; ?>" />
    </div>
  </div>
...
function addStore<?php echo $shipping_id; ?>() {
	...
	var html  = '<div class="tab-pane" id="tab-store-<?php echo $shipping_id; ?>'+store_row<?php echo $shipping_id; ?>+'">';
		...
		html += '      <thead>';
		html += '        <tr>';
		html += '          <td class="text-left"><?php echo $entry_sort_order; ?></td>';
...
function addAddress<?php echo $shipping_id; ?>(row) {
	var html  = '<tr>';
		html += '  <td class="text-left"><input type="text" name="super<?php echo $shipping_id; ?>_store['+row+'][store]['+s_row<?php echo $shipping_id; ?>+'][sort_order]" value="" placeholder="<?php echo $entry_sort_order; ?>" class="form-control" /></td>';
