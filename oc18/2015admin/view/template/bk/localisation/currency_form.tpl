      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-currency" class="form-horizontal">
		  ...
		  <div class="form-group">
			...
			<div class="col-sm-10">
			  <input type="text" name="country" value="" placeholder="<?php echo $entry_country; ?>" id="input-country" class="form-control" />
			  <div id="country" class="well well-sm" style="height: 150px; overflow: auto;">
				<?php foreach ($countries as $country) { ?>
				<div id="country<?php echo $country['country_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $country['name']; ?>
				  <input type="hidden" name="country[]" value="<?php echo $country['iso_code_2']; ?>" />
				</div>
				<?php } ?>
			  </div>
			</div>
...
$('input[name=\'country\']').autocomplete({
	...
	'select': function(item) {
		...
		$('#country' + item['value']).remove();
		
		$('#country').append('<div id="country' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="country[]" value="' + item['value'] + '" /></div>');	
	}
});

$('#country').delegate('.fa-minus-circle', 'click', function() {
	$(this).parent().remove();
});
--></script>
<?php echo $footer; ?>