...
class ModelReportAdvSaleProfitExportAll extends Model {
	public function getSaleProfitExportAll($data = array()) { 	
		...
		$sql .= "), 2)) AS order_sales, 
		(ROUND(o.currency_value*(IFNULL((SELECT SUM(op.cost*op.quantity) FROM `" . DB_PREFIX . "order_product` op WHERE op.order_id = o.order_id),0)+o.commission";
		if ($this->config->get('adv_profit_reports_formula_sop2')) {																																																																																																																																																																																																																																												   			$sql .= "+o.shipping_cost";
		}
		if ($this->config->get('adv_profit_reports_formula_sop3')) {		
			$sql .= "+o.payment_cost";
		}
			$sql .= "), 2)) AS order_costs, 
		(ROUND(o.currency_value*((IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'sub_total' GROUP BY ot.order_id),0)+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'handling' GROUP BY ot.order_id),0)+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'low_order_fee' GROUP BY ot.order_id),0)+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'reward' GROUP BY ot.order_id),0)+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'coupon' GROUP BY ot.order_id),0)+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'credit' GROUP BY ot.order_id),0)+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'voucher' GROUP BY ot.order_id),0)";
		if ($this->config->get('adv_profit_reports_formula_sop1')) {																																																																																																																																																																																																																																												   			$sql .= "+IFNULL((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'shipping' GROUP BY ot.order_id),0)";
		}
		$sql .= ")";					
		...
		$sql .= ")), 2),0)) AS order_profit_margin_percent, 
		...
		(SELECT (SELECT IF (o.customer_id = 0,'0',CONCAT('<a href=\"index.php?route=sale/customer/edit&token=$token&customer_id=',o.customer_id,'\">',o.customer_id,'</a>')) FROM `" . DB_PREFIX . "order` o WHERE op.order_id = o.order_id GROUP BY o.customer_id) FROM `" . DB_PREFIX . "order_product` op WHERE op.order_id = o.order_id GROUP BY op.order_id) AS customer_cust_id, 
	...
	public function getSaleProfitExportAllExcel($data = array()) { 	
		$query = $this->db->query("SET SESSION group_concat_max_len=500000");
		...
		(o.currency_value*op.total) AS product_sales, 
		(o.currency_value*op.cost*op.quantity) AS product_costs, 
		(o.currency_value*(op.total - (op.cost*op.quantity))) AS product_profit, 
 	  	(100*(op.total - (op.cost*op.quantity)) / op.total) AS product_profit_margin_percent, 
		...
		(SELECT o.currency_value*SUM(op.cost*op.quantity) FROM `" . DB_PREFIX . "order_product` op WHERE op.order_id = o.order_id) AS order_product_costs, 
